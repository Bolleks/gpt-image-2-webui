...
const WORDLIST = [
  ...
  'brisk','broad','broccoli','broken','bronze','broom','brother','brown',
  ...
  'candy','canyon','capable','capital','captain','car','carbon','card',
  ...
  'child','chimney','choice','choose','chronic','chunk','church','cigar',
  ...
  'coil','coin','collect','column','combine','comfort','comic','common',
  'company','concert','conduct','confirm','congress','connect','consider',
  'control','convince','cook','cool','copper','coral','corn','correct',
  ...
  'emerge','emotion','employ','empower','empty','enable','enact','encounter',
  ...
  'final','find','fine','finger','finish','fire','firm','fiscal',
  ...
  'grand','grant','grape','grass','gravity','great','green','grid',
  ...
  'maple','marble','march','margin','marine','market','marriage','material',
  'math','matrix','matter','maximum','maze','meadow','mean','measure',
  'meat','media','melody','melt','member','memory','mention','menu',
  ...
  'pave','payment','peace','peanut','pear','pen','peasant','pelican',
  ...
  'risk','ritual','rival','river','roast','robot','robust','rocket',
  ...
  'salon','salty','sample','sand','satisfy','satoshi','sauce','sausage',
  ...
  'unhappy','uniform','unique','unit','unknown','unlock','until','unusual',
  'unveil','update','upgrade','uphold','upon','upper','upset','urban',
  'usage','use','used','useful','useless','utility','vacant','vacuum',
...
export function seedPhraseToUserId(seedPhrase: string): string {
  const hash = createHash('sha256').update(seedPhrase.normalize('NFKC').trim().toLowerCase()).digest('hex');
  return hash.slice(0, 8);
}

export function seedPhraseToHash(seedPhrase: string): string {
  return createHash('sha256').update(seedPhrase.normalize('NFKC').trim().toLowerCase()).digest('hex');
}

export function validateSeedPhrase(phrase: string): { valid: boolean; error?: string } {
  const words = phrase.normalize('NFKC').trim().toLowerCase().split(/\s+/);
  if (words.length !== 12) {
    return { valid: false, error: 'Seed-фраза должна содержать 12 слов' };
  }
  for (const word of words) {
    if (!WORDLIST.includes(word)) {
      return { valid: false, error: `Слово "${word}" не найдено в словаре` };
    }
  }
  return { valid: true };
}